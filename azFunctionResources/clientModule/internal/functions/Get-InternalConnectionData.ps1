...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'FunctionAppDeploymentTools.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'FunctionAppDeploymentTools.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'FunctionAppDeploymentTools.Client.ProtectedToken'
