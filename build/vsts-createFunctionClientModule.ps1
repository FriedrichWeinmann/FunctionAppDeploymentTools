...
$parentModule = 'FunctionAppDeploymentTools'
if (-not $ModuleName) { $ModuleName = 'FunctionAppDeploymentTools.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\FunctionAppDeploymentTools'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\FunctionAppDeploymentTools.psd1"
$prereqHash = @{
